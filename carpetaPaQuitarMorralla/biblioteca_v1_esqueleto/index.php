...
function mostrarListaLibros(){
  ...
  if($result = $db->query("SELECT * FROM libros
    INNER JOIN escriben ON libros.idLibros = escriben.idLibros
    ...
    echo "Erro al tratar de recuperar los datos de la base de datos.";
  }
...
function formularioModificarLibro()
{
  ...
  $db = new mysqli("db", "root", "test", "biblioteca");
  $result = $db->query("SELECT ilLibro,titulo,genero,pais,ano,numPaginas");

  //AQUI FALTA FECTH ASSOC ESTÁ ARRIBA
  $titulo = $_REQUEST["titulo"];
  $genero = $_REQUEST["genero"];
  $pais = $_REQUEST["pais"];
  $ano = $_REQUEST["ano"];
  ...
  Género:<input type='text' name='genero'>".$genero."<br>
  País:<input type='text' name='pais'>".$pais."<br>
  Año:<input type='text' name='ano'>".$ano."<br>
  Número de páginas:<input type='text' name='numPaginas'><br>";
